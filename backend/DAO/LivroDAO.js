...
class LivroDAO {
  ...
  async criar(livro) {
    try {
      ...
      await connection.query(
        'INSERT INTO livro (titulo, autor, ano_publicacao, categoria, disponivel) VALUES (?, ?, ?, ?, true)',
  ...
  async atualizar(id, livro) {
    try {
      const connection = await this.pool.getConnection();
      await connection.query(
        'UPDATE livro SET titulo = ?, autor = ?, ano_publicacao = ?, categoria = ?, disponivel = ? WHERE id_livro = ?',
        [
          livro.titulo,
          livro.autor,
